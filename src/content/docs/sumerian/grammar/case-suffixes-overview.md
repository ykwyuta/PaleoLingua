...
| 能格 | `-e` | エ | 他動詞の主語（動作主） |
| 属格 | `-ak` | アク | 所有・限定（「AのB」） |
| 与格 | `-ra` | ラ | 動作の受け手・受益者（主に人・神） |
| 処格 | `-a` | ア | 場所（「〜において」、主に無生物） |
| 向格（処格・方向格） | `-e` | エ | 方向・到達点（「〜へ」、能格と同形だが機能が異なる） |
| 終端格 | `-še3` | シェ | 到達点・目的（「〜まで」「〜のために」） |
| 奪格・具格 | `-ta` | タ | 起点・手段（「〜から」「〜によって」） |
| 共格 | `-da` | ダ | 同伴（「〜と一緒に」） |
| 等格 | `-gin7` | ギン | 比較・類似（「〜のように」、索引番号7は読まない） |
