---
title: Numbers and Counting
description: Sumerian's sexagesimal (base-60) number system, its cuneiform numeral signs, and the words for the numbers 1-10.
topic: sumerian
sidebar:
  order: 4
---

## The sexagesimal system

Sumerian numbers are not built on a base-10 system but on a "sexagesimal" (base-60) system, combined with base-10 elements in a mixed structure. Traces of this base-60 system survive indirectly in units we still use today — 60 minutes to an hour, 60 seconds to a minute, 360 degrees in a circle.

## Cuneiform numeral signs

In early cuneiform, numbers were written with dedicated signs whose shape and size varied depending on the value they represented.

| Number | Sign shape |
|---|---|
| 1 | a small vertical wedge (a round impression made by pressing the stylus tip in at an angle) |
| 10 | a Winkelhaken (a small angular notch) |
| 60 | the same shape as 1, but a larger impression |
| 600 | the same shape as 10, but a larger impression |
| 3,600 (60×60) | an even larger circular impression |

In other words, each power of 60 reused the same basic shapes as 1 and 10, distinguished mainly by size.

## The words for 1-10

Separate from the numeral signs, Sumerian also has spoken number words.

| Number | Sumerian | Cuneiform |
|---|---|---|
| 1 | diš (aš) | 𒁹 |
| 2 | min | 𒈫 |
| 3 | eš5 | 𒐈 |
| 4 | limmu | 𒐉 |
| 5 | ia2 | 𒐊 |
| 6 | aš3 | 𒐋 |
| 7 | imin | 𒐌 |
| 8 | ussu | 𒐍 |
| 9 | ilimmu | 𒑆 |
| 10 | u | 𒌋 |

These number words underlie the constant quantity expressions found in administrative and economic texts — "N sheep," "N tablets," and so on.

<div class="plc-column">
  <img class="plc-avatar" src="/images/characters/sumerian/boy-f02.png" alt="En" />
  <div class="plc-body">
    <p class="plc-title">💬 Learning Column: Sumer Is Still Ticking Inside Your Watch</p>
    <p class="plc-line"><b>En:</b> Nin, every time we say "meet in 60 minutes," we're quietly relying on the Sumerian number system.</p>
    <p class="plc-line"><b>Nin:</b> 60 minutes to an hour, 60 seconds to a minute — none of that is a tidy base-10 number, yet it's stuck around this whole time. And a circle being 360 degrees is a multiple of 60 too.</p>
    <p class="plc-line"><b>En:</b> Sixty divides evenly by 2, 3, 4, 5, 6, and more — which is probably exactly why it was such a practical choice for fraction-heavy calculations.</p>
    <p class="plc-line"><b>Nin:</b> Kind of romantic, actually — 4,000-year-old math quietly ticking away inside every wristwatch.</p>
  </div>
</div>

## See also

- [Script & Phonology: Writing Direction and Wedge Shapes](/en/sumerian/script-and-phonology/cuneiform-writing-direction/)
- [Readings & Examples: Reading Your First Sentence](/en/sumerian/readings/first-sentence-royal-inscription/)
