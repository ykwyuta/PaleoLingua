---
title: Overview of Case Suffixes
description: A summary table of Sumerian's main case suffixes — absolutive, ergative, genitive, dative, locative, and more — as a map of the whole system.
topic: sumerian
sidebar:
  order: 4
---

## Sumerian's case-marking system

Sumerian is agglutinative: it attaches case suffixes to the end of a noun phrase to mark the role that phrase plays in the clause (agent, patient, location, direction, and so on). Terminology and exact groupings vary somewhat between grammarians, but the following cases are widely recognized.

| Case | Suffix | Main function |
|---|---|---|
| Absolutive | unmarked (∅) | subject of intransitive verbs, object of transitive verbs |
| Ergative | `-e` | subject (agent) of transitive verbs |
| Genitive | `-ak` | possession/specification ("B's A") |
| Dative | `-ra` | recipient/beneficiary of an action (mainly humans and gods) |
| Locative | `-a` | location ("in/at," mainly for inanimates) |
| Locative-terminative (directive) | `-e` | direction/goal ("onto/towards" — same form as the ergative but a distinct function) |
| Terminative | `-še3` | goal or purpose ("to/until," "for the sake of") |
| Ablative-instrumental | `-ta` | source or means ("from," "by means of") |
| Comitative | `-da` | accompaniment ("together with") |
| Equative | `-gin7` | comparison/similarity ("like") |

## Where the case suffix attaches

Case suffixes attach at the very end of the noun phrase order already introduced (noun → adjective → genitive phrase → demonstrative → plural marker → case suffix). No matter how long the phrase grows, only a single case suffix attaches — to the phrase as a whole, not to each word inside it.

```
e2   lugal-ak-e
house king-GEN-ERG
"the king's house (as agent, e.g. of building something)"
```

## Where this article fits

Each case is treated in more depth in its own dedicated article (the genitive, dative, terminative, locative, and so on). For now, use this table as a map of the whole system: Sumerian marks grammatical roles with a fixed set of case suffixes, each with its own function.

<div class="plc-column">
  <img class="plc-avatar" src="/images/characters/sumerian/girl-f02.png" alt="Nin" />
  <div class="plc-body">
    <p class="plc-title">💬 Learning Column: Case Suffixes Are Luggage Tags</p>
    <p class="plc-line"><b>Nin:</b> Ten different case suffixes sounds a little intimidating.</p>
    <p class="plc-line"><b>En:</b> The trick is remembering: one noun phrase, one tag, stuck on at the very end. In `e2 lugal-ak-e`, "house" and "king" are strung together, but only a single `-e` gets attached at the back — just like one luggage tag at an airport.</p>
    <p class="plc-line"><b>Nin:</b> So it's basically like Japanese particles — "ga," "no," "ni" — slapped onto the end of the whole phrase.</p>
    <p class="plc-line"><b>En:</b> Once you see it that way, ten cases just means ten kinds of destination label. Not so scary after all, right?</p>
  </div>
</div>

## See also

- [Basic Grammar: Sumerian Ergative and Absolutive Case](/en/sumerian/grammar/sumerian-ergative/)
- [Basic Grammar: The Genitive Case -ak](/en/sumerian/grammar/genitive-case-ak/)
- [Basic Grammar: Dative, Terminative, Locative, and Other Cases](/en/sumerian/grammar/dative-terminative-locative/)
