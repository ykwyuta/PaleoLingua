...
`topic` はトップページの記事一覧（言語ごとにタブ切り替えできる一覧、`src/components/ArticleList.astro`）が
その言語のマスコット画像を各記事行のアイコンとして表示するために使うキーで、`src/content.config.ts` の
`TOPICS` に列挙された値のいずれかでなければならない（新しい言語を追加する場合はそこにも追記する）。
「準備中の記事です」
...
### 4.1 楔形文字（原字）とカタカナ読みの併記（必須）

対象言語が楔形文字を用いる言語（シュメール語、アッカド語）の場合、翻字を伴うすべての例文・語彙表・パラダイムには、
次の4点を **必ずセットで** 示すこと。いずれか一つでも欠けた例文は未完成として扱う。

1. **楔形文字（原字、Unicode）** — 実際の楔形文字グリフ（U+12000–U+123FF の Cuneiform ブロック、または
   U+12400 以降の数字記号ブロック）。
2. **翻字（ラテン翻字）** — 既存記事と同じ表記慣習（ハイフンで形態素境界、上付きの限定符、下付きの索引番号）。
3. **カタカナ読み**（日本語版のみ） — 索引番号を無視した実際の発音のカタカナ表記。英語版にはカタカナ読みの行は不要。
4. **形態素グロス／訳**。

**マークアップ（サイト全体で統一）：**

- 例文ブロックでは、楔形文字グリフをコードブロックの中に入れず、コードブロックの直前に
  `<p class="cuneiform">` 段落として独立させる（`src/styles/custom.css` の `.cuneiform` クラスが
  専用フォント・文字サイズを適用する）。単語間はスペースで区切り、下に続くコードブロックの語と大まかに
  位置が揃うようにする。コードブロック自体は「翻字」「カタカナ読み（日本語版のみ）」「グロス／意味」のみを含む。
  ...
  英語版も同じ `<p class="cuneiform">` 行を使い、コードブロックからカタカナ行のみ省く。

- 語彙表・逐語グロス表など、1行が1語（1形態素）に対応する表には、翻字の列のすぐ後ろ（カタカナ読みの
  列より前）に「楔形文字」（英語版は「Cuneiform」）という列を追加し、各行にその語のグリフを入れる。
...
- 例文・表の直後の解説文中で、翻字をバッククォートで参照する箇所（例：`` `lugal` ``）には、可能な範囲で
  直後にグリフを添える（例：`` `lugal` 𒈗（王）``）。これは本文全体の全出現箇所に機械的に付けるのではなく、
  その例文を直接説明している文（グロス表の直後の段落など）に限ってよい。

**楔形文字グリフの正確性について**：楔形文字のUnicodeコードポイントは記憶だけで書くと誤りやすい（似た字形・
似た読みのサインを取り違えるリスクが高い）。執筆時は必ず一次資料で裏取りすること。具体的には、Oracc Sign List
（`https://github.com/oracc/osl` の `00lib/osl.asl`）を取得し、`@sign <SIGN NAME>` ブロック内の `@ucun`
行（実グリフ）と `@v` 行（その字形が持つ読み一覧）を突き合わせて、使いたい翻字（例: `du3`, `mu`, `lugal`）に
対応するグリフを特定する。読み仮名は下付き数字（`du₃` 等）に正規化してから `@v` の値と照合する。該当する読みが
一覧に見当たらない、または字形に自信が持てない場合は、その語を無理に使わず、確実に裏取りできる別の語彙・例文に
差し替えること（不確かなグリフを推測で載せるより、確実な少数の例に絞る方がよい）。合字（`|X.Y|` 形式）は複数の
実サインを連結して表示してよい。
...
シュメール語アセット（`sumerian/`）と同じ構成・ファイル名で配置し、この表を更新すること。具体的には
男の子・女の子それぞれ以下の計10枚（両者で計20枚）、いずれも白背景を透過処理したPNGとする。

- `boy-01.png` / `boy-02.png` / `boy-03.png` / `boy-base.png` — 全身立ち絵（ポーズ違い）。
- `boy-f01.png` 〜 `boy-f06.png` — 表情違いの顔クローズアップ6種（喜怒哀楽などバリエーションを持たせる）。
- `girl-01.png` / `girl-02.png` / `girl-03.png` / `girl-base.png`、`girl-f01.png` 〜 `girl-f06.png` —
  女の子側も同様。

`f01`〜`f06` の顔クローズアップは、7.3節の学びのコラムのアバターだけでなく、トップページの記事一覧
（`ArticleList.astro`）の各記事行のアイコンにも使われる。同コンポーネントは記事スラッグの
ハッシュ値から6種の表情の中を自動で振り分けて選ぶため、同じ画像の使い回しに見えないようにする狙いが
ある。**この自動振り分けが機能するには、新しい言語でも `f01`〜`f06` の6種すべてを男女とも用意する必要
がある**（1〜2種類しか用意されていないとアイコンの多様性が失われる）。キャラクターの名前はその言語で
意味のある基本単語（シュメール語の en/nin のように）を選ぶと、名前自体が語彙学習のヒントになるため
望ましい。
...
- 新しい言語の `topic` を初めて使う場合は `src/content.config.ts` の `TOPICS`、`astro.config.mjs` の
  ...
  `src/content/docs/en/index.mdx` の「記事一覧」/「Articles」セクション（`<Tabs>` に言語ごとの
  `<TabItem>` を1個追加し、その中にカテゴリ見出し＋`<ArticleList>` 3個）にも追記されている。
- プレースホルダーでない実記事には、ja/en 両方に「7. 学びのコラム」のルールに従ったコラムが挿入されている。
- 楔形文字を用いる言語の記事では、翻字を伴う例文・語彙表すべてに「4.1」の楔形文字＋翻字（＋日本語版のみ
  カタカナ読み）が揃っており、かつ楔形文字グリフが Oracc Sign List 等で裏取り済みである。
